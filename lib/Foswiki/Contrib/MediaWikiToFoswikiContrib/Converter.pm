...
$translationToken2 = "\2";
$attachmentTemplate =
'%META:FILEATTACHMENT{name="%file%" attachment="%file%" attr="" comment="%comment%" date="%date%" path="%file%" size="%size%" stream="%file%" user="%user%" version="1"}%';
...
sub new {
    ...
    # open dump
    if ( $this->{fileName} ) {
        die "ERROR: file '$this->{fileName}' not found"
          unless -f $this->{fileName};
    ...
    foreach my $lang (
        'bs', 'da',     'de', 'el', 'es', 'fr', 'gl', 'hr',
        'it', 'lt',     'hu', 'nl', 'ja', 'no', 'pl', 'pt',
        'sq', 'simple', 'sr', 'fi', 'sv', 'th', 'zh', 'tr',
        'eo', 'he'
      )
    {
...
sub convert {
    ...
    while ( defined( my $page = $this->{pages}->next ) ) {
        ...
        # create
        next
          unless $this->createPage( $page, $text )
              || $this->createRedirect( $page, $text );
...
sub createPage {
    ...
    if ( $this->{seenPage}{$webTopicName} ) {
        $this->writeWarning(
"'$mwTitle' clashes with '$this->{seenPage}{$webTopicName}' on '$webTopicName'"
        );
    }
...
sub saveTopic {
    ...
    my $defaultWebFileName =
        $Foswiki::cfg{DataDir} . '/'
      . $this->{defaultWeb} . '/'
      . $topic . '.txt';

    if ( -f $topicFileName && !-f $defaultWebFileName && !$this->{cummulative} )
    {    # overwriting default topics is ok
        my $index = 0;
        my $newTopicFileName;
        ...
        $this->writeWarning(
"woops $topicFileName.txt already exists ... renaming it to $newTopicFileName"
        );
...
sub convertMarkup {
    ...
    # refs (aka footnotes)
    $_[0] =~
      s/<ref name="(.+?)">(.+?)<\/ref>/$this->handleFootNote($page, $2, $1)/ges;
    $_[0] =~ s/<ref>(.+?)<\/ref>/$this->handleFootNote($page, $1)/ges;

    # gallery
    $_[0] =~
      s/<gallery(.*?)?>(.*?)<\/gallery>/$this->handleGallery($page, $1, $2)/ges;

    # multimedia
    $_[0] =~
      s/\[\[$this->{language}{Image}:(.+)\]\]/$this->handleImage($page, $1)/ge;
    $_[0] =~
      s/\[\[$this->{language}{Media}:(.+)\]\]/$this->handleMedia($page, $1)/ge;

    # mailto
    $_[0] =~ s/\[mailto:([^\s]+?)\]/$1/g;
    $_[0] =~
      s/\[mailto:(.*?) (.*?)\]/\[${translationToken0}\[mailto:$1\]\[$2\]\]/g;

    # exteral link
    $_[0] =~
s/\[?\[((?:https?|ftp)\:.+?)(?:[ \|]+(.+?))?\]\]?/$this->handleExternalLink($page, $1, $2)/ge;

    # file
    $_[0] =~
s/\[?\[(file\:.+?)(?:[ \|]+(.+?))?\]\]?/$this->handleExternalLink($page, $1, $2)/ge;

    # internal links
    $_[0] =~ s/\[\[(.+?)\]\]/$this->handleInternalLink($page, $1)/ge;

    # templates
    $_[0] =~ s/{{{\b([^\}]+?)\b}}}/$this->handleTemplateVariable($page, $1)/ge
      ;    # variables
    1 while $_[0] =~
          s/{{(?!.*{{)(.+?)}}/$this->handleTemplateCall($page, $1)/ges
    ;      # includes
    ...
    if ($mode) {
        ...
        $this->handleVerbatim( $page, $_[0] )
          if $mode && $_[0] =~ /(^|[\n\r]) /;

        # indentation
        $this->handleIndentation( $page, $_[0] )
          if $mode && $_[0] =~ /(^|[\n\r]):/;
    }
...
sub handleTemplateCall {
    ...
    my %variables = (
        ...
        'CURRENTTIMESTAMP' =>
          '%DISPLAYTIME{"$year$month$day$hour$minutes$seconds"}%',
...
# will create a link to Main:B while linking to Template:B in Foswiki
    ...
    my $result =
      '%INCLUDE{"' . $webTopicName . '"';    # TODO: how do we get the templates
...
sub attachMedia {
    ...
    my $utf8file = $file;
    $utf8file =
      to_utf8( -string => $utf8file, -charset => $Foswiki::cfg{Site}{CharSet} )
    ...
    unless ( -d $pubDir ) {
        if ( $this->{dry} ) {
            ...
            unless ( mkdir $pubDir ) {
                $this->writeWarning(
                    "failed to attach data to topic $webTopicName");
    ...
    $target =
      from_utf8( -string => $target, -charset => $Foswiki::cfg{Site}{CharSet} )
...
sub getPageText {
    ...
    my $text = ${ $page->text };
    $text =
      from_utf8( -string => $text, -charset => $Foswiki::cfg{Site}{CharSet} )
      unless $Foswiki::cfg{Site}{CharSet} =~ /^utf-?8$/i;
