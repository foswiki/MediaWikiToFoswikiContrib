...
sub handleBefore {
    ...
    # own image handler
    $_[0] =~
s/\[\[$this->{language}{Image}:(.+?)\]\]/$converter->handleImage($page, $1)/ge;
}
...
sub handleFinal {
    ...
    foreach my $catName ( keys %seenCategories ) {
        if ( !defined( $converter->{categories}{$catName} ) ) {
            $converter->writeWarning(
                "seen $catName ... but not defined ... creating it");
            my $cat = {
                title => $catName,
...
'%META:PREFERENCE{name="DENYTOPICVIEW" title="DENYTOPICVIEW" type="Set" value=""}%'
    ...
    # save it
    $converter->saveTopic( $cat->{page}, $template, $converter->{targetWeb},
        $cat->{topic} );
